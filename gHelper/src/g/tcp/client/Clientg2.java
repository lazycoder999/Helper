...
//					}
//					
//					String dateString = new SimpleDateFormat("hh:mm:ss.S ").format(lastPingReceivedTms);
//					// ClientGui.lbllastPingTimeTmsText = "lastPingTimeTms = " + dateString;
//				}
//			}
//		});
		...
		Thread receivePingThread = new Thread(new Runnable() {
			public void run() {
				while (true) {
					...
					if (System.currentTimeMillis() - lastPingReceivedTms > 10000) {
						Gh.prnt("Client ping not reveiced over 10 000ms");
						lastPingReceivedTms = System.currentTimeMillis() - 8000;
	...
	public void reconnecter(final String caller) {
		if (ip != null && !ip.equals("")) {
			if (!reconnecterCalled) {
				...
				Gh.prnt("Client reconnecter caller=" + caller);
